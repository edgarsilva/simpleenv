...
func TestLoadBehaviorCases(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "optional with min present still validates",
			fieldType:   reflect.TypeOf(int(0)),
			tag:         "SIMPLEENV_TEST_OPTIONAL_MIN_PRESENT;optional;min=2",
			envValue:    strPtr("1"),
			wantErr:     true,
			errContains: []string{"a value >= 2"},
		},
		{
			name:        "optional with oneof present still validates",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_OPTIONAL_ONEOF_PRESENT;optional;oneof=dev,prod",
			envValue:    strPtr("staging"),
			wantErr:     true,
			errContains: []string{"one of [dev,prod]"},
		},
		{
		...
		{
			name:      "trimspace with oneof trims before validation",
			fieldType: reflect.TypeOf(""),
			tag:       "SIMPLEENV_TEST_TRIMSPACE_ONEOF;trimspace;oneof=dev,prod",
			envValue:  strPtr("  dev  "),
			wantValue: "dev",
		},
		{
			name:      "minlen and maxlen both succeed",
			fieldType: reflect.TypeOf(""),
			tag:       "SIMPLEENV_TEST_LEN_BOTH;minlen=3;maxlen=10",
			envValue:  strPtr("simpleenv"),
			wantValue: "simpleenv",
		},
		{
			name:        "minlen fails when value too short",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_LEN_MIN_FAIL;minlen=4",
			envValue:    strPtr("abc"),
			wantErr:     true,
			errContains: []string{"length >= 4"},
		},
		{
			name:        "maxlen fails when value too long",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_LEN_MAX_FAIL;maxlen=3",
			envValue:    strPtr("abcd"),
			wantErr:     true,
			errContains: []string{"length <= 3"},
		},
		{
			name:      "trimspace applies before minlen",
			fieldType: reflect.TypeOf(""),
			tag:       "SIMPLEENV_TEST_LEN_TRIMSPACE;trimspace;minlen=3",
			envValue:  strPtr("  abc  "),
			wantValue: "abc",
		},
		{
			name:        "allowempty does not bypass minlen",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_LEN_ALLOWEMPTY;allowempty;minlen=1",
			envValue:    strPtr(""),
			wantErr:     true,
			errContains: []string{"length >= 1"},
		},
		{
			name:        "trimspace without allowempty rejects whitespace-only",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_TRIMSPACE_EMPTY;trimspace",
			envValue:    strPtr("   \t\n  "),
			wantErr:     true,
			errContains: []string{"non-empty value"},
		},
		{
			name:      "trimspace with allowempty accepts whitespace-only",
			fieldType: reflect.TypeOf(""),
			tag:       "SIMPLEENV_TEST_TRIMSPACE_EMPTY_ALLOWED;trimspace;allowempty",
			envValue:  strPtr("   \t\n  "),
			wantValue: "",
		},
		{
			name:        "allowempty does not bypass other rules",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_OPTIONAL_EMPTY_ALLOWED_ONEOF;optional;allowempty;oneof=dev,prod",
			envValue:    strPtr(""),
			wantErr:     true,
			errContains: []string{"one of [dev,prod]"},
		},
		...
		{
			name:        "trimspace on int is invalid",
			fieldType:   reflect.TypeOf(int(0)),
			tag:         "SIMPLEENV_TEST_TRIMSPACE_INT;trimspace",
			envValue:    strPtr("12"),
			wantErr:     true,
			errContains: []string{"trimspace is only supported"},
		},
		{
			name:        "minlen on int is invalid",
			fieldType:   reflect.TypeOf(int(0)),
			tag:         "SIMPLEENV_TEST_MINLEN_INT;minlen=1",
			envValue:    strPtr("12"),
			wantErr:     true,
			errContains: []string{"minlen/maxlen are only supported"},
		},
		{
			name:        "invalid minlen value returns error",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_MINLEN_INVALID;minlen=nope",
			envValue:    strPtr("abc"),
			wantErr:     true,
			errContains: []string{"valid non-negative integer"},
		},
		{
			name:        "negative maxlen value returns error",
			fieldType:   reflect.TypeOf(""),
			tag:         "SIMPLEENV_TEST_MAXLEN_NEGATIVE;maxlen=-1",
			envValue:    strPtr("abc"),
			wantErr:     true,
			errContains: []string{"valid non-negative integer"},
		},
		{
			name:        "min does not trim whitespace automatically",
			fieldType:   reflect.TypeOf(int(0)),
			tag:         "SIMPLEENV_TEST_MIN_WHITESPACE;min=1",
			envValue:    strPtr(" 2 "),
			wantErr:     true,
			errContains: []string{"numeric value for min comparison"},
		},
		{
